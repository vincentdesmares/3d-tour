...
var createCurve = function createCurve(scene, path, name) {
  var curve = new _three.QuadraticBezierCurve3(
    new _three.Vector3(path.startX, path.startY, path.startZ),
    new _three.Vector3(path.controlX, path.controlY, path.controlZ),
    new _three.Vector3(path.endX, path.endY, path.endZ)
  )
...
exports.default = function(scene, sceneState) {
  for (var screenId in sceneState.screens) {
    var screen = sceneState.screens[screenId]
