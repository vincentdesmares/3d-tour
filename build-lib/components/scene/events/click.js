...
exports.default = function(renderer, threeState, sceneState, dispatch) {
  function onclick(event) {
    threeState.camera.updateMatrixWorld()
    threeState.raycaster.setFromCamera(threeState.mouse, threeState.camera)
    var intersects = threeState.raycaster.intersectObjects(
      threeState.intersectables
    )
    if (intersects.length > 0) {
      ...
      try {
        for (
          var _iterator = intersects[Symbol.iterator](), _step;
          ...
          if (intersect.object.name) {
            ...
            if (intersect.object.name.search("action@") !== -1) {
              ...
              var action = sceneState.actions[actionName]
              dispatch(action)
              threeState.camera.lookAt(new _three.Vector3(0, 20, -20))
            }
          } else {
